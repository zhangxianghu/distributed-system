//mpirun -n 2 ./pan.out fl_compact.tab  fl_compact_part.2 5 2
#include "stdio.h"
#include "stdlib.h"
...
void initialization(char * fileName){
    ...
    credit = (double *)calloc((total_rounds+1)*largest +1, sizeof(double));
...
int mpi_send_credit(int round_number){
	...
	int j = 0;
	for(i=1;i<=largest;i++){
		if(credit[round_number*largest+i]!=(double)0){
...
int main(int argc, char ** argv){
	...
	begin = clock();			//Start time of reading file
	...
	end = clock();				//Finish time of reading file
